// Minimal SHA-256, used only to fingerprint the rules file.
//
// The hash is provenance, not security: it goes into /rules and into the reload
// log so an operator can confirm which bytes produced the policy version a
// decision cites. It runs once per reload on the control-plane thread.
#pragma once
...
class Sha256 {
 public:
  ...
  [[nodiscard]] std::string hex() {
    ...
    for (int i = 0; i < 8; ++i) {
      for (int b = 0; b < 4; ++b) {
        const auto byte = static_cast<std::uint8_t>(h_[i] >> (24 - 8 * b));
        out[static_cast<std::size_t>(i * 8 + b * 2)] = kHex[byte >> 4];
        out[static_cast<std::size_t>(i * 8 + b * 2 + 1)] = kHex[byte & 0x0F];
      }
    }
    return out;
  }

  static std::string hash_hex(std::string_view s) {
    Sha256 h;
    h.update(s);
    return h.hex();
  }
